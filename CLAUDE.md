...
- `submission` (`src/modules/submission`) — `/collaborators/:collaboratorId/documents/:documentTypeId/submissions` (submit a new document version + list version history) and `/documents/pending` (paginated, filterable list of links still awaiting submission). Submitting runs in a `Serializable`-isolated Prisma transaction that deactivates the current version and creates the next one; a unique index on `(collaboratorDocumentTypeId, version)` turns concurrent submits into a 409 (`Prisma.PrismaClientKnownRequestError` code `P2002` or `P2034`) instead of corrupting version history.
...
- Soft-deleting a `Collaborator` or `DocumentType` cascades a soft delete to its `CollaboratorDocumentType` links — don't assume deleting the parent leaves orphaned active links.
...
- `npm run test:e2e` (config in `jest.e2e.config.js`, specs in `e2e/*.e2e.test.ts`) hits a real Postgres database — **the `db` container from `docker-compose.yml` must already be running (`docker compose up -d`) before running it**, and `.env.test` must exist (copy from `.env.test.example`). `e2e/setup/globalSetup.ts` creates the dedicated `api_documentacao_colaboradores_test` database (if missing) and applies migrations against it automatically, but it still needs a reachable Postgres server to connect to first.
